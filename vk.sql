...
DROP TABLE IF EXISTS users; -- П о л ь з о в а т е л ь
CREATE TABLE users (
...
DROP TABLE IF EXISTS `profiles`; -- П р о ф е л ь п о л ь з о в а т е л я
...
DROP TABLE IF EXISTS messages; -- С о о б щ е н и я 
...
DROP TABLE IF EXISTS friend_requests; -- З а п р о с ы   в  Д р у з ь я
...
DROP TABLE IF EXISTS communities; -- Г р у п п ы
...
DROP TABLE IF EXISTS users_communities; -- Г р у п п ы  П о л ь з о в а т е л я
...
DROP TABLE IF EXISTS post_users; -- П о с т ы  п о  л ь з о в а т е л я
...
DROP TABLE IF EXISTS post_likes; -- Л а й к и  П о с т а м
CREATE TABLE post_likes(id SERIAL,
    user_id BIGINT UNSIGNED NOT NULL,
    post_users_id BIGINT UNSIGNED NOT NULL,
    created_at DATETIME DEFAULT NOW(),
    
    FOREIGN KEY (user_id) REFERENCES users(id),
    FOREIGN KEY (post_users_id) REFERENCES post_users(id)
    );
